...
dx_total <- clean_data %>% 
  ...
  add_count(short_dx = substr(dx, 1, 5)) %>%  #look only at first 5 characters in the ICD codes
...
clean_data %>% 
  ...
  mutate(top_dx =
    case_when(dx1_n >= dx2_n ~ dx1_icd10, 
              TRUE ~ dx2_icd10) #bring back the more common diagnosis
  )
...
rank_data <- clean_data %>% 
  ...
  select(visit_key, accurate_prediction_ind, discharge_prediction, days_until_discharge, diff, gest_age_groups, sex, race, payor_group, pred_order, central_line_status, ett_status, iv_fluid_or_med_start_post_prediction) #select variables
...
print(all_variables) #all variables

all_variables %>% 
  filter(p_val <= .05) %>% #filter for significant with p<0.05
...
#all of these variables are associated with accuracy
...
summary(glm(accurate_prediction_ind ~ discharge_prediction + gest_age_groups + sex + race + payor_group + central_line_status + ett_status + iv_fluid_or_med_start_post_prediction, data = rank_data_first, family = binomial))
#no significant mediators of accuracy on first estimates

rank_data_last <- rank_data %>%
  filter(pred_order == "last")
summary(glm(accurate_prediction_ind ~ discharge_prediction + gest_age_groups + sex + race + payor_group + central_line_status + ett_status + iv_fluid_or_med_start_post_prediction, data = rank_data_last, family = binomial))
...
summary(lm(diff ~ discharge_prediction + gest_age_groups + sex + race + payor_group + pred_order + central_line_status + ett_status + iv_fluid_or_med_start_post_prediction, data = rank_data_correct)) 
#gestational age >35 weeks, prediction order, and ETT status are associated with how off we are in our estimations
